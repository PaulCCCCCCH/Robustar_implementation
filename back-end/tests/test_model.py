...
class TestModel:
    class TestModelUpload:
        ...
        def test_model_upload2(self, client):
            # TODO:
            response = dummy_api_upload_dummy_model(client, "test-resnet-18-2")
            assert response.status_code == 200

    class TestModelSwitch:
        def test_model_upload2(self, client):
            # TODO:
            response = dummy_api_upload_dummy_model(client, "dummy-model-1")
            assert response.status_code == 200
    ...
    #         # Upload two models
    #         resp = dummy_api_upload_dummy_model(client, "dummy-model-1")
    #         assert (
    #             resp.status_code == 200
    #         ), f"Fail to upload dummy model. {resp.get_json().get('detail')}"
    #         resp = dummy_api_upload_dummy_model(client, "dummy-model-2")
    #         assert (
    #             resp.status_code == 200
    #         ), f"Fail to upload dummy model. {resp.get_json().get('detail')}"
