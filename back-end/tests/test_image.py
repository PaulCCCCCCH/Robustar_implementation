...
class TestImage:
    class TestGetImageList:
        ...
        def test_get_image_list_success(self, client):
            ...
            assert [x[0] for x in rv["data"]] == [
                f"{RServer.getServer().baseDir}/dataset/train/bird/{idx}.JPEG"
                for idx in range(4)
            ]
            assert rv["code"] == 0
            ...
            rv = response.get_json()
            assert [x[0] for x in rv["data"]] == [
                f"{RServer.getServer().baseDir}/dataset/train/turtle/{idx}.JPEG"
                for idx in range(10)
            ]
            assert rv["code"] == 0
            assert rv["msg"] == "Success"
            ...
            rv = response.get_json()
            assert [x[0] for x in rv["data"]] == [
                f"{RServer.getServer().baseDir}/dataset/test/turtle/{idx}.JPEG"
                for idx in range(4, 10)
            ]
            assert rv["code"] == 0
            assert rv["msg"] == "Success"
            ...
            rv = response.get_json()
            assert [x[0] for x in rv["data"]] == [
                f"{RServer.getServer().baseDir}/dataset/validation/turtle/{idx}.JPEG"
    ...
    class TestGetNextImage:
        ...
        def test_get_next_image_success(self, client):
            ...
            rv = response.get_json()
            assert (
                rv["data"] == RServer.getServer().baseDir + "/dataset/train/bird/2.JPEG"
            )
            assert rv["code"] == 0
            assert rv["msg"] == "Success"
            # TODO: split `annotated` and `proposed`
