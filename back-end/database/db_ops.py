...
def db_insert(db_conn: Connection, table_name: str, keys: Tuple[str], values: Tuple):
    ...
    template = "INSERT INTO {} ({}) values ({})".format(
        table_name, ",".join(keys), ",".join(["?" for _ in values])
    )
...
def db_insert_many(
    db_conn: Connection, table_name: str, keys: Tuple[str], values: List[Tuple]
):
    if len(values) == 0:
        return
    assert len(keys) == len(
        values[0]
    ), "key and value array does not contain the same number of attributes"
...
def db_update_by_path(
    db_conn: Connection, table_name: str, path: str, keys: Tuple[str], values: Tuple
):
    assert len(keys) == len(values), "key and value array length not equal"
    db_cursor = db_conn.cursor()

    template = "UPDATE {} SET {} WHERE path=?".format(
        table_name, ",".join(["{} = ?".format(key) for key in keys])
    )

    db_cursor.execute(template, values + (path,))


def db_update_many_by_paths(
    db_conn: Connection,
    table_name: str,
    paths: List[str],
    keys: Tuple[str],
    ...
    db_cursor.executemany(
        template, [values + (path,) for (values, path) in zip(values_list, paths)]
    )
...
def db_delete_by_path(db_conn: Connection, table_name: str, path: str):
    ...
    template = "DELETE FROM {} WHERE path=?".format(table_name)
...
def db_delete_all(db_conn: Connection, table_name: str):
    ...
    template = "DELETE FROM {}".format(table_name)
...
def db_count_all(db_conn: Connection, table_name: str):
    ...
    template = "SELECT COUNT (*) FROM {}".format(table_name)
...
def db_get_cls_result(db_conn: Connection, table_name: str, correct: bool):
    ...
    template = "SELECT * from {} WHERE classified=?".format(table_name)
